...
namespace InputLogic
{
    internal class MouseManager
    {
        ...
        private static ViGEmInput? vigemInput;
        ...
        public static void InitViGEm()
        {
            if (vigemInput != null) return;
            try
            {
                vigemInput = new ViGEmInput();
            }
            catch (Exception ex)
            {
                System.Windows.MessageBox.Show($"Failed to connect to ViGEmBus:\n{ex.Message}", "ViGEm Error");
            }
        }

        private static void SendMove(int x, int y, int click)
        {
            string method = Dictionary.dropdownState.ContainsKey("Mouse Movement Method") ? Dictionary.dropdownState["Mouse Movement Method"] : "Arduino";

            if (method == "Arduino")
            {
                arduinoInput?.SendMouseCommand(x, y, click);
            }
            else if (method == "ViGEmBus")
            {
                int xSpeed = (int)(Dictionary.sliderSettings.ContainsKey("ViGEmBus X Speed") ? Dictionary.sliderSettings["ViGEmBus X Speed"] : 150);
                int ySpeed = (int)(Dictionary.sliderSettings.ContainsKey("ViGEmBus Y Speed") ? Dictionary.sliderSettings["ViGEmBus Y Speed"] : 150);
                vigemInput?.SendMouseCommand(x, y, click, xSpeed, ySpeed);
            }
        }
        ...
        public static void MoveCrosshair(int detectedX, int detectedY)
        {
            ...
            SendMove(newPosition.X, newPosition.Y, 0);
